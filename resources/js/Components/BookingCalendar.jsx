...
export default function BookingCalendar({ venue, initialBookedDates }) {
    ...
    useEffect(() => {
        ...
        const fp = flatpickr(datePickerRef.current, {
            ...
            minDate: "today",
            dateFormat: "Y-m-d",
            locale: language === 'en' ? english : French,
            disable: bookedDates,
    ...
    }, [bookedDates, venue.price_per_day, language]);
    ...
    return (
        <div className="sticky top-24 bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-gray-100">
            <div className="mb-6 flex items-end gap-2 border-b border-gray-100 pb-6">
                ...
                <span className="text-gray-500 font-medium mb-1">{t('booking_calendar.per_day')}</span>
            </div>
            ...
            {user && user.id === venue.user_id ? (
                <div className="text-center py-6">
                    ...
                    <h3 className="text-lg font-bold text-gray-900 mb-2">{t('booking_calendar.your_space')}</h3>
                    <p className="text-sm text-gray-500 mb-6">
                        {t('booking_calendar.cannot_book')}
                    </p>
                    <a href={route('dashboard')} className="btn bg-amber-500 hover:bg-amber-600 text-white w-full rounded-xl py-3 block text-center font-bold shadow-lg shadow-amber-500/30">
                        {t('booking_calendar.go_dashboard')}
                    </a>
                </div>
            ) : (
                <form onSubmit={submitBooking}>
                    {/* Date Picker */}
                    <div className="mb-4">
                        <label className="block text-sm font-bold text-gray-900 mb-2 uppercase tracking-wide">{t('booking_calendar.planned_dates')}</label>
                        <div className="relative">
                            ...
                            <input 
                                ...
                                placeholder={t('booking_calendar.select_dates')} 
                                readOnly 
                            />
                        </div>
                    ...
                    {/* Event Type */}
                    <div className="mb-4">
                        <label className="block text-sm font-bold text-gray-900 mb-2 uppercase tracking-wide">{t('booking_calendar.event_type')}</label>
                        <select 
                        ...
                        >
                            <option value="Fête">{t('booking_calendar.type_party')}</option>
                            <option value="Mariage">{t('booking_calendar.type_wedding')}</option>
                            <option value="Conférence">{t('booking_calendar.type_conference')}</option>
                            <option value="Shooting">{t('booking_calendar.type_shooting')}</option>
                            <option value="Autre">{t('booking_calendar.type_other')}</option>
                        </select>
                    ...
                    {/* Guests */}
                    <div className="mb-6">
                        <label className="block text-sm font-bold text-gray-900 mb-2 uppercase tracking-wide">{t('booking_calendar.guest_count')}</label>
                        <div className="relative">
                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        ...
                        <p className="text-xs text-gray-500 mt-2 text-right">{t('booking_calendar.max_people').replace('{capacity}', venue.capacity)}</p>
                        {errors.guest_count && <p className="text-red-500 text-xs mt-1">{errors.guest_count}</p>}
                    </div>

                    {/* Special requests */}
                    <div className="mb-6">
                        <label className="block text-sm font-bold text-gray-900 mb-2 uppercase tracking-wide">{t('booking_calendar.special_requests')}</label>
                        <textarea 
                            ...
                            rows="2"
                            placeholder={t('booking_calendar.requests_placeholder')}
                    ...
                    >
                        {processing ? t('booking_calendar.processing') : t('booking_calendar.book_space')}
                    </button>
                    
                    <p className="text-center text-xs text-gray-400 mt-4">
                        {t('booking_calendar.no_charge')}
                    </p>
                </form>
            )}
        </div>
