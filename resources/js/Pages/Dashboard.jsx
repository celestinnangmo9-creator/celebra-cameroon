...
export default function Dashboard({ auth, userRole, userVenues, totalVenues, totalBookings, totalRevenue, pendingBookingsCount, recentBookings, upcomingAppointments, venuePerformances }) {
    ...
    const renderHostDashboard = () => (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('dashboard.host.overview')}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('dashboard.host.overview_desc')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 flex items-center space-x-4">
                    ...
                    <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('dashboard.host.revenue')}</p>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 flex items-center space-x-4">
                    ...
                    <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('dashboard.host.my_venues')}</p>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 flex items-center space-x-4">
                    ...
                    <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('dashboard.host.total_bookings')}</p>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 flex items-center space-x-4">
                    ...
                    <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('dashboard.host.pending')}</p>
            ...
            {venuePerformances && venuePerformances.length > 0 && (
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/50 p-6 mb-6 mt-6">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">{t('dashboard.host.my_halls')}</h3>
                        ...
                        {venuePerformances.map(venue => (
                            <div key={venue.id} className="border border-gray-100 dark:border-gray-700 p-4 rounded-xl flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{venue.city} • {venue.bookings_count} {t('dashboard.host.bookings_count')}</p>
                                </div>
                                <div className="flex gap-2">
                                    <Link href={route('venues.edit', venue.id)} className="bg-gray-50 text-gray-600 px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors font-bold text-xs flex items-center gap-1">
                                        <i className="fa-solid fa-pen"></i> {t('dashboard.host.edit')}
                                    </Link>
                                    <Link href={route('venues.stats', venue.id)} className="bg-emerald-50 text-emerald-600 px-3 py-2 rounded-lg hover:bg-emerald-100 transition-colors font-bold text-xs flex items-center gap-1">
                                        <i className="fa-solid fa-chart-pie"></i> {t('dashboard.host.stats')}
                                    </Link>
                                </div>
                            </div>
                        ))}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/50 overflow-hidden">
                <div className="p-6 border-b border-gray-100 dark:border-gray-700/50 flex justify-between items-center">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('dashboard.host.recent_requests')}</h3>
                    <Link href={route('bookings.index')} className="text-sm font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">{t('dashboard.host.see_all')}</Link>
                </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50 dark:bg-gray-900/50 text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400">
                                    <th className="px-6 py-4 font-semibold">{t('dashboard.host.table.client')}</th>
                                    <th className="px-6 py-4 font-semibold">{t('dashboard.host.table.venue')}</th>
                                    <th className="px-6 py-4 font-semibold">{t('dashboard.host.table.period')}</th>
                                    <th className="px-6 py-4 font-semibold">{t('dashboard.host.table.amount')}</th>
                                    <th className="px-6 py-4 font-semibold">{t('dashboard.host.table.status')}</th>
                                    <th className="px-6 py-4 font-semibold text-right">{t('dashboard.host.table.actions')}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                                {recentBookings.map((booking) => (
                                    <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-sm text-gray-900 dark:text-white">{t('dashboard.host.table.from')} {new Date(booking.start_date).toLocaleDateString('fr-FR')}</div>
                                            <div className="text-sm text-gray-500">{t('dashboard.host.table.to')} {new Date(booking.end_date).toLocaleDateString('fr-FR')}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            `}>
                                                {booking.status === 'confirmed' ? t('dashboard.host.table.confirmed') : (booking.status === 'pending' ? t('dashboard.host.table.pending_status') : t('dashboard.host.table.cancelled'))}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right text-sm font-medium">
                                            <Link href={route('bookings.index')} className="text-emerald-600 hover:text-emerald-900 dark:hover:text-emerald-400 mr-3">{t('dashboard.host.table.manage')}</Link>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
    ...
    const renderClientDashboard = () => (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('dashboard.client.welcome').replace('{name}', auth.user.name)}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('dashboard.client.welcome_desc')}</p>
                </div>
                <Link href={route('venues.index')} className="inline-flex items-center rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/30 transition-all hover:-translate-y-1 hover:bg-emerald-700 hover:shadow-emerald-600/40">
                    ...
                    {t('dashboard.client.search_venue')}
                </Link>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/50 overflow-hidden p-8 text-center">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">{t('dashboard.client.no_bookings')}</h3>
                <p className="mt-1 text-sm text-gray-500">{t('dashboard.client.no_bookings_desc')}</p>
            </div>
        </div>
    );
